...
const ActiveDispatches: React.FC<ActiveDispatchesProps> = ({ className = '' }) => {
  ...
  const openIncidentDrawer = useCallback((incident: Incident, isNew = false) => {
  ...
  const loadRemainingPagesInBackground = useCallback(async (startUrl: string, since: string) => {
    ...
    while (currentUrl) {
      try {
        ...
        // Silently add the new incidents to the list
  ...
  const fetchFirstPage = async () => {
    try {
      ...
      const since24HoursAgo = formatDateForFirstDue(new Date(Date.now() - 24 * 60 * 60 * 1000))
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to fetch dispatches'
  ...
  useEffect(() => {
    console.log('Incidents updated:', incidents.length, 'Previous IDs:', previousIncidentIds.current.size, 'Sound enabled:', soundEnabled)
    
    if (incidents.length === 0) return // Skip on initial load
    
    const currentIds = new Set(incidents.map(incident => incident.dispatch.id))
    ...
    console.log('New incidents found:', newIncidents.length, newIncidents.map(i => i.dispatch.id))
    
    // Check for brand new dispatches with our units
    const newOurUnitDispatches = newIncidents.filter(incident => 
      isOurUnit(incident.dispatch.unit_codes) &&
      incident.dispatch.status_code === 'open'
    )
    
    if (newOurUnitDispatches.length > 0 && previousIncidentIds.current.size > 0) {
      // Show drawer for the first new dispatch with our units
      const newDispatch = newOurUnitDispatches[0]
      console.log('New dispatch with our units detected:', newDispatch.dispatch.id)
      openIncidentDrawer(newDispatch, true)
    }
    
    if (newIncidents.length > 0 && previousIncidentIds.current.size > 0 && soundEnabled) {
  ...
  if (loading && incidents.length === 0) {
    return (
      <div className={`bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 ${className}`}>
        <h2 className="text-xl font-semibold mb-4 text-gray-800 dark:text-white">Dispatches</h2>
  ...
  if (error) {
    return (
      <div className={`bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 border-l-4 border-red-500 ${className}`}>
        <h2 className="text-xl font-semibold mb-4 text-gray-800 dark:text-white">Dispatches</h2>
  ...
  const openIncidents = incidents
    ...
    .sort((a, b) => {
      ...
      return new Date(b.dispatch.created_at).getTime() - new Date(a.dispatch.created_at).getTime()
    })
  
  const closedIncidents = incidents.filter(({ dispatch }) => dispatch.status_code === 'closed')

  return (
    <div className={`bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 flex flex-col ${className}`}>
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-semibold text-gray-800 dark:text-white">Dispatches</h2>
        <div className="flex items-center space-x-2">
          ...
          <button 
            onClick={() => {
              ...
              localStorage.setItem('soundNotifications', newSoundEnabled.toString())
            }}
            className={`p-1 rounded ${soundEnabled ? 'text-blue-600 dark:text-blue-400' : 'text-gray-400 dark:text-gray-600'} hover:text-gray-700 dark:hover:text-gray-200`}
